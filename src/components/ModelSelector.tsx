...
import { Settings, ChevronDown, Loader2, AlertCircle } from 'lucide-react';
...
function ModelSelector({ models, onChange, isExpanded, onToggle, isLoadingModels, modelsError }: ModelSelectorProps) {
    ...
    return (
      <div className="space-y-2">
        ...
        <select
          ...
          className="w-full bg-purple-700/50 border border-purple-600/50 text-purple-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
        >
    ...
    <div className="bg-purple-800/30 rounded-xl border border-purple-700/50">
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-purple-700/50 mt-4 pt-4">
          {modelsError && (
            <div className="bg-orange-900/30 border border-orange-600/50 rounded-lg p-3 flex items-start space-x-2">
              <AlertCircle className="w-4 h-4 text-orange-400 mt-0.5 flex-shrink-0" />
              <p className="text-sm text-orange-200">{modelsError}</p>
            </div>
          )}
          ...
          {isLoadingModels ? (
            <div className="flex items-center justify-center py-8 space-x-2">
              ...
              <span className="text-purple-300">Fetching available models...</span>
            </div>
          ) : (
            <>
              ...
              <button
                ...
                disabled={isLoadingModels}
                className="w-full px-4 py-2 bg-purple-600/50 hover:bg-purple-600/70 text-purple-100 rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Reset to Defaults
              </button>
            </>
